 ...
 * <p>
 * 思考：
 * 为防止字符串中存在重复的字符导致排列重复，采用set来判断是否用过某个字符
 * 固定字符串的每个字符，共有N的阶乘中排列情况（字符串没有重复字符）
 */
public class Solution {
    ...
    /**
     * @param x     字符数组中被固定的字符下标
     * @param chars 字符数组
     * @param res   排列结果
     */
    private void dfs(int x, char[] chars, List<String> res) {
        //递归结束条件-x到达最后一个字符时
        //if的判断语句也可以写成 x == chars.length ，由于固定最后一个字符时只有一种排列情况，相当于阶乘最后乘的是1，所以x==chars.length-1即可
        if (x == chars.length - 1) {
            //添加排列
        ...
        //set中存储的是拼接的字符，这个for循环可以让chars中每个字符都出现在拼接后的字符串的每一位（x表示字符串的第几位）
        ...
        //！！！回溯时x也会变为初始值，比如for循环中i到了=x+1的这个循环了，则此时x也为初始值0了，例如“abc”，i=b了，会在接下来的出现，索引0，1
        //处交换变成bac,接着索引0，2处交换变成cab,然后再恢复递归结束，再恢复交换变成abc,再然后i++,变成索引3处的c,如上类推（0，2交换，接着1，2交换）
